...
async def process_questions(message: Message):
    ...
    if len(questions) == 0:
        ...
        await message.answer('Չկան հարցեր.')
...
async def process_answer(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.message.answer('Գրեք պատասխանը.', reply_markup=ReplyKeyboardRemove())
...
async def process_submit(message: Message, state: FSMContext):
    ...
    await message.answer('Համոզվեք, որ ձեր պատասխանում չեք սխալվել.', reply_markup=submit_markup())
...
async def process_send_answer(message: Message, state: FSMContext):
    await message.answer('Չեղարկված!', reply_markup=ReplyKeyboardRemove())
    await state.finish()
...
async def process_send_answer(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        text = f'Հարց: <b>{question}</b>\n\nՊատասխան: <b>{answer}</b>'

        await message.answer('Ուղարկված է!', reply_markup=ReplyKeyboardRemove())
